...
class Planner(Node):
    def __init__(self):
        ...
        # # subscribe to topic /odom to get the robot's current position
    ...
    def planner_loop(self):
        ...
        # v_attractive_x = self.ka * (difference_x / distance_to_goal)
        # v_attractive_y = self.ka * (difference_y / distance_to_goal)
        ...
        for i, r in enumerate(ranges):
            ...
            if r < self.p_0 and r > 0.01:
                ...
                v_repulsive_x += self.kr * \
                    (1.0/r - 1.0/self.p_0) / (r**2) * math.cos(angle)
                v_repulsive_y += self.kr * \
                    (1.0/r - 1.0/self.p_0) / (r**2) * math.sin(angle)
        ...
        twist.linear.x = max(min(velocity_x_robot, 0.5), -0.5)

        # Also we can use the velocity_y_robot for angular velocity direction
        # twist.angular.z = max(min(velocity_y_robot, 1.0), -1.0)
        twist.angular.z = math.sin(velocity_y_robot) * 1.5
